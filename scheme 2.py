...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if check_procedure(operator):
            return scheme_eval(operator.scheme_eval(rest,env), env)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            return self.bindings.get(symbol,0)
...
def make_let_frame(bindings, env):
    ...
    formal_param = Pair(clause.first, nil)
    variable = Pair(scheme_eval(clause.second.first,env),nil)
    bindings = bindings.second
    while bindings != nil:
        clause = bindings.first
        check_form(clause,2,2)
        formal_param = Pair(clause.first, formal_param)
        variable = Pair(scheme_eval(clause.second.first,env),variable)
        check_formals(formal_param)
        bindings = bindings.second
    
    return env.make_child_frame(formal_param,variable)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        return apply_macro(target.first,env)
