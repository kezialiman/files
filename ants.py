...
class Place(object):
    """A Place holds insects and has an exit to another Place."/Users/kezialiman/desktop/cs61a/projects/ants"""
...
class Bee(Insect):
    ...
    damage = 1
    is_watersafe = True
    direction = 1 #1 means forward, 0 means backward
    is_scared = False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def helper(ant_place,range):
            if ant_place == hive:
                return None
            elif ant_place.bees and ant_place != hive and self.min_range <= range <= self.max_range:
                return random_or_none(ant_place.bees)
            else:
                return helper(ant_place.entrance, range+1)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        list_bees = list(self.place.bees)
        self.armor -= amount
        if self.armor <= 0:
            self.place.remove_insect(self)
            for b in list_bees:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.queen_placeholder == []:
            self.true_queen = True
            QueenAnt.queen_placeholder = ['Occupied']
        else:
    ...
    def action(self, colony):
        ...
        if self.true_queen == False:
            ...
            while behind:
                if behind.ant != None:
                    if behind.ant not in self.double_dmg:
                        behind.ant.damage *= 2
                        self.double_dmg.append(behind.ant)
                    if behind.ant.is_container and behind.ant.contained_ant and behind.ant.contained_ant not in self.double_dmg:
                        behind.ant.contained_ant.damage *= 2
                        self.double_dmg.append(behind.ant.contained_ant)
                behind = behind.exit
    ...
    def action_helper(colony):
        ...
        if duration > 0:
            ...
            if duration == 0 and effect == make_scare:
                bee.direction = 1 #bee move forward again
            previous_action(colony) #go back to bee's original action
        duration -= 1
